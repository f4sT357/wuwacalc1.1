...
5.  **データ検証・補正**:
    - OCRの読み間違い（例: `11.6%` を `116` と読み間違える等）を自動補正。
    - **改善**: 数値の範囲補正（Range Validation）を単位判定（%かフラットか）より先に実行することで、誤判定による補正スキップを防止し、精度を向上。
...
> [!NOTE]
> 初心者・中級者向けの使いやすさを優先し、現在は **Normalized** と **CV** をデフォルトの表示・計算対象としています。高度な計算設定（Ratio, Roll, Effective）はUI上から非表示、または簡略化されています。
...
## 6. テストと品質管理
- **テストスイート**: `unittest` をベースとした包括的なユニットテストを整備。
- **実行環境**: `run_all_tests.py` により、OCRパース、スコア計算、UIコンポーネント、履歴管理などの全29項目（2026年2月時点）を一括テスト可能。
- **品質維持**: UI仕様の変更やロジックの改善に合わせてテストコードを常に同期させ、デグレードを防止。

---

## 7. 開発履歴と移行
- **PyQt6 → PySide6**: 2025年12月、LGPLライセンスへの準拠とメンテナンス性の向上のため、完全に移行済み。
- **UI簡略化**: 2026年2月、ユーザー体験向上のため、ログタブや高度な計算オプションをデフォルトで隠す「シンプルモード」への移行を実施。
- **ドキュメント更新**: 最新のテストスイート状況とロジック改善を反映。
